...
const DEMO_CARDS = [
  ...
  {id:'p6d', name:'Mew ex',                  set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:45,    paid:50,   qty:1, folder:'f2', num:'151/165'},
...
const CardImages = {
  ...
  // One Piece images: use the Limitless PUBLIC CDN (limitless3.nyc3.cdn — NOT the
  // broken private bucket limitlesstcg.nyc3). Falls back to official site.
  _onePiece(card) {
    if (!card.num || !/^OP\d{2}-\d{3}$/i.test(card.num)) return null;
    const num = card.num.toUpperCase();
    const set = num.split('-')[0];
    // Public Limitless CDN — this subdomain has CORS open
    return `https://limitless3.nyc3.cdn.digitaloceanspaces.com/op/${set}/${num}_EN.webp`;
  },
  ...
  _sealed(card) {
    ...
    if (card.game === 'onepiece') {
      ...
      if (match) {
        ...
        return `https://limitless3.nyc3.cdn.digitaloceanspaces.com/op/${setId}/${setId}-EN.webp`;
      }
    }
    return null;
  },
      ...
      grids.forEach(grid => {
        ...
        grid.querySelectorAll(`[data-card-id="${card.id}"] .thumb`).forEach(thumb => {
          ...
          if (card.game === 'onepiece' && card.num && /^OP\d{2}-\d{3}$/i.test(card.num)) {
            const num = card.num.toUpperCase();
            const set = num.split('-')[0];
            // Try public Limitless CDN first, then official site as fallback
            const srcs = [
              `https://limitless3.nyc3.cdn.digitaloceanspaces.com/op/${set}/${num}_EN.webp`,
