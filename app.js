...
const DEMO_CARDS = [
  ...
  {id:'p13',name:'Umbreon VMAX Alt Art',    set:'Evolving Skies',   game:'pokemon',  cat:'slab',   grader:'BGS',grade:9,   market:1800, paid:1450, qty:1, folder:'f3', num:'215/203'},
...
const DEMO_COLLECTION = {
  portfolios:[
    {id:'cp1',name:'My Collection',listed:false},
  ],
  cards:[
    ...
    {id:'cc2',name:'Umbreon VMAX Alt Art',set:'Evolving Skies',   game:'pokemon', cat:'slab',  grader:'PSA',grade:9,   market:1750, paid:1400, qty:1, portfolio:'cp1', num:'215/203'},
    ...
    {id:'cc4',name:'Monkey D. Luffy',     set:'OP01 Romance Dawn',game:'onepiece',cat:'raw',   grader:null, grade:null, market:42,   paid:35,   qty:1, portfolio:'cp1', num:'OP01-060'},
    {id:'cc5',name:'Yamato Full Art',     set:'OP04 Kingdoms',    game:'onepiece',cat:'raw',   grader:null, grade:null, market:35,   paid:28,   qty:2, portfolio:'cp1', num:'OP04-110'},
  ]
};
...
async function loadPriceChart(card, days) {
  ...
  const marketPrice = card.market;
  const startPrice  = card.paid > 0 ? card.paid : marketPrice * (0.88 + ((card.id||'').split('').reduce((a,c)=>a+c.charCodeAt(0),0) % 15) / 100);
  // Build deterministic but natural-looking path from startPrice→marketPrice
  const seed = (card.id||'').split('').reduce((a,c)=>a+c.charCodeAt(0),0);
  const pts  = [];
  for (let i = days; i >= 0; i--) {
    const progress = (days - i) / days; // 0 at start, 1 at end
    const base = startPrice + (marketPrice - startPrice) * progress;
    // Small deterministic noise — never overwhelms the true trend
    const noise = base * 0.03 * Math.sin(seed * 0.1 + i * 0.7);
    const price  = Math.max(base + noise, marketPrice * 0.1);
    const d = new Date(); d.setDate(d.getDate() - i);
    pts.push({ date: d.toLocaleDateString('en-CA',{month:'short',day:'numeric'}), price: +price.toFixed(2) });
  }
  // Pin the last point to exact market price
  pts[pts.length - 1].price = marketPrice;
  const labels = pts.map(p => p.date);
  const data   = pts.map(p => p.price);
  const trend  = marketPrice >= startPrice;
  ...
  S.priceChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { mode: 'index', intersect: false, backgroundColor: '#1A2744', borderColor: 'rgba(43,92,230,0.3)', borderWidth: 1, titleColor: '#6B7A99', bodyColor: '#fff', callbacks: { label: c => `${fmt(c.raw)}` } },
      },
      scales: {
        x: { display: false },
        y: { display: true, grid: { color: 'rgba(43,92,230,0.06)' }, ticks: { color: '#6B7A99', font: { family: 'Nunito', size: 10, weight: '700' }, callback: v => fmt(v, 0) } }
      }
    }
  });
}
