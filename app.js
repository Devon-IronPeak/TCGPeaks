...
const CardImages = {
  ...
  // ROOT CAUSE FIX: Limitless CDN is inaccessible from browsers (CORS/private bucket).
  // Primary: official One Piece card game site — reliable & public.
  _onePiece(card) {
    if (!card.num || !/^OP\d{2}-\d{3}$/i.test(card.num)) return null;
    const num = card.num.toUpperCase();
    return `https://en.onepiece-cardgame.com/images/cardlist/card/${num}.png`;
  },
  ...
  _sealed(card) {
    ...
    if (card.game === 'onepiece') {
      ...
      if (match) {
        ...
        // Official site booster art
        return `https://en.onepiece-cardgame.com/images/products/booster/${setId.toLowerCase()}.jpg`;
      }
    }
    return null;
  },
      ...
      grids.forEach(grid => {
        ...
        grid.querySelectorAll(`[data-card-id="${card.id}"] .thumb`).forEach(thumb => {
          ...
          if (card.game === 'onepiece' && card.num && /^OP\d{2}-\d{3}$/i.test(card.num)) {
            // Official site is primary; no CORS issues, works reliably
            const num = card.num.toUpperCase();
            img.src = `https://en.onepiece-cardgame.com/images/cardlist/card/${num}.png`;
