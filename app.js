...
const DEMO_CARDS = [
  ...
  // Charizard ex 125/197 Double Rare ~$18 raw
  ...
  // Iono Full Art 254/193 ~$14. Note: 185/193 is the $0.27 common — wrong num was in old data
  {id:'p4a', name:'Iono',                    set:'Paldea Evolved',     game:'pokemon', cat:'raw',   grader:null,  grade:null, market:14,    paid:18,   qty:2, folder:'f1', num:'254/193'},
  ...
  // ── Pokémon · 151 & Vintage (f2) ──
  {id:'p5',  name:'Pikachu IR',               set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:67,    paid:25,   qty:3, folder:'f2', num:'173/165'},
  {id:'p6',  name:'Blastoise ex',              set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:14,    paid:14,   qty:2, folder:'f2', num:'186/165'},
  {id:'p6a', name:'Venusaur ex',               set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:12,    paid:14,   qty:1, folder:'f2', num:'184/165'},
  {id:'p6b', name:'Charizard ex',              set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:45,    paid:40,   qty:1, folder:'f2', num:'185/165'},
  {id:'p6c', name:'Mewtwo ex',                 set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:25,    paid:18,   qty:2, folder:'f2', num:'205/165'},
  {id:'p6d', name:'Mew ex',                    set:'151',                game:'pokemon', cat:'raw',   grader:null,  grade:null, market:55,    paid:50,   qty:1, folder:'f2', num:'204/165'},
  ...
  // Iono SAR 249/193 PSA 10 ~$280 (SAR, not 185 the common)
  ...
  {id:'jp5', name:'Iono SAR (JPN)',              set:'Clay Burst',               game:'pokemon',cat:'raw',grader:null,grade:null,market:20,paid:16,qty:3,folder:'f8',num:'096/071',lang:'Japanese'},
  ...
  // CORRECTED: OP01-024 = Luffy SR char, OP01-001 = Zoro Leader, OP01-120 = Shanks SEC
  {id:'o1',  name:'Monkey D. Luffy',             set:'OP01 Romance Dawn',  game:'onepiece',cat:'raw',grader:null,grade:null,market:8,  paid:12, qty:2,folder:'f5',num:'OP01-024'},
  {id:'o2',  name:'Roronoa Zoro',                set:'OP01 Romance Dawn',  game:'onepiece',cat:'raw',grader:null,grade:null,market:4,  paid:6,  qty:1,folder:'f5',num:'OP01-001'},
  {id:'o3',  name:'Nami',                        set:'OP01 Romance Dawn',  game:'onepiece',cat:'raw',grader:null,grade:null,market:2,  paid:4,  qty:3,folder:'f5',num:'OP01-016'},
  {id:'o3a', name:'Shanks',                      set:'OP01 Romance Dawn',  game:'onepiece',cat:'raw',grader:null,grade:null,market:35, paid:45, qty:1,folder:'f5',num:'OP01-120'},
  {id:'o3aa',name:'Yamato',                      set:'OP01 Romance Dawn',  game:'onepiece',cat:'raw',grader:null,grade:null,market:18, paid:22, qty:1,folder:'f5',num:'OP01-121'},
  {id:'o3b', name:'Portgas D. Ace',              set:'OP02 Paramount War', game:'onepiece',cat:'raw',grader:null,grade:null,market:30, paid:32, qty:2,folder:'f5',num:'OP02-013'},
  {id:'o3c', name:'Sanji',                       set:'OP01 Romance Dawn',  game:'onepiece',cat:'raw',grader:null,grade:null,market:3,  paid:5,  qty:2,folder:'f5',num:'OP01-013'},

  // ── One Piece · OP04–OP07 (f6) ──
  ...
  // ── One Piece · Graded (f7) ──
  {id:'o8',  name:'Monkey D. Luffy',             set:'OP01 Romance Dawn',            game:'onepiece',cat:'slab',grader:'PSA',grade:10, market:260,paid:290,qty:1,folder:'f7',num:'OP01-024'},
  ...
  {id:'o10', name:'Shanks SEC',                  set:'OP01 Romance Dawn',            game:'onepiece',cat:'slab',grader:'PSA',grade:9,  market:220,paid:180,qty:1,folder:'f7',num:'OP01-120'},
...
const DEMO_COLLECTION = {
  ...
  cards:[
    ...
    {id:'cc4',name:'Monkey D. Luffy',            set:'OP01 Romance Dawn',            game:'onepiece',cat:'raw', grader:null, grade:null, market:8,    paid:12,  qty:1, portfolio:'cp1', num:'OP01-024'},
    ...
    {id:'cc7',name:'Pikachu IR',                   set:'151',                          game:'pokemon', cat:'raw', grader:null, grade:null, market:67,   paid:25,  qty:1, portfolio:'cp2', num:'173/165'},
...
const CardImages = {
  ...
  // One Piece images — original working approach restored.
  // Uses the Limitless CDN with EN suffix, falls back to official Bandai site.
  _onePiece(card) {
    if (!card.num || !/^OP\d{2}-\d{3}$/i.test(card.num)) return null;
    const num = card.num.toUpperCase();
    const set = num.split('-')[0];
    return `https://limitlesstcg.nyc3.cdn.digitaloceanspaces.com/one-piece/${set}/${num}_EN.webp`;
  },
  ...
  _sealed(card) {
    ...
    if (card.game === 'onepiece') {
      ...
      if (match) {
        ...
        return `https://limitlesstcg.nyc3.cdn.digitaloceanspaces.com/one-piece/${setId}/${setId}_DISPLAY_EN.webp`;
      }
    }
    return null;
  },
        ...
        grid.querySelectorAll(`[data-card-id="${card.id}"] .thumb`).forEach(thumb => {
          ...
          if (card.game === 'onepiece' && card.num && /^OP\d{2}-\d{3}$/i.test(card.num)) {
            ...
            const srcs = [
              ...
              `https://limitlesstcg.nyc3.cdn.digitaloceanspaces.com/one-piece/${set}/${num}.webp`,
              `https://en.onepiece-cardgame.com/images/cardlist/card/${num}.png`,
            ];
            let attempt = 0;
            const tryNext = () => {
              if (attempt >= srcs.length) { delete IMG_CACHE[card.id]; img.replaceWith(ph); return; }
...
/* ══ LIVE PRICE REFRESH ══
   Fetches current TCGPlayer market prices from pokemontcg.io for all store cards
   that have a known set code. Runs once on load, silently updates tiles.
   This is what makes prices accurate — the DEMO_CARDS values are just seeds. */
const PRICE_CACHE_KEY = 'tcgp_live_prices';
const PRICE_CACHE_TTL = 6 * 3_600_000; // 6 hours
...
async function refreshLivePrices() {
  ...
  try {
    ...
    if (cached && (Date.now() - cached.ts) < PRICE_CACHE_TTL) {
      applyPriceCache(cached.prices);
      return;
    }
  } catch {}

  // Build a deduplicated list of (setCode, cardNumber) pairs to fetch
  const setMap = CardImages._POKE_SETS;
  const toFetch = [];
  const seen = new Set();
  S.store.cards.filter(c => c.game === 'pokemon' && c.num && c.cat !== 'sealed' && c.cat !== 'case').forEach(c => {
    const setCode = setMap[c.set];
    if (!setCode) return;
    const rawNum = c.num.split('/')[0];
    const key = `${setCode}::${rawNum}`;
    if (!seen.has(key)) { seen.add(key); toFetch.push({ id: c.id, setCode, rawNum, cardIds: [c.id] }); }
    else {
      // attach extra card IDs that share same set+num (e.g. same card listed multiple times)
      const existing = toFetch.find(f => f.setCode === setCode && f.rawNum === rawNum);
      if (existing) existing.cardIds.push(c.id);
    }
  });
  ...
  const priceMap = {}; // cardId -> market price
  const batches = [];
  for (let i = 0; i < toFetch.length; i += 10) batches.push(toFetch.slice(i, i + 10));

  for (const batch of batches) {
    // Build a query like: (set.id:sv2 number:254) OR (set.id:sv3pt5 number:25) ...
    const q = batch.map(b => `(set.id:${b.setCode} number:${b.rawNum})`).join(' OR ');
    try {
      const res = await fetch(
        `https://api.pokemontcg.io/v2/cards?q=${encodeURIComponent(q)}&select=id,set,number,tcgplayer,cardmarket&pageSize=100`,
        { signal: AbortSignal.timeout(8000) }
      );
      const data = await res.json();
      (data.data || []).forEach(apiCard => {
        const setCode = apiCard.set?.id;
        const rawNum  = apiCard.number;
        if (!setCode || !rawNum) return;
        // Extract best market price across ALL price keys
        const prices = apiCard.tcgplayer?.prices || {};
        const allMarkets = Object.values(prices).map(p => p?.market || 0).filter(v => v > 0);
        const tcgMkt = allMarkets.length > 0 ? Math.max(...allMarkets) : 0;
        const cmAvg  = apiCard.cardmarket?.prices?.averageSellPrice || 0;
        const mkt = tcgMkt || (cmAvg ? Math.round(cmAvg * 1.08 * 100) / 100 : 0);
        if (mkt > 0) {
          const match = toFetch.find(f => f.setCode === setCode && f.rawNum === rawNum);
          if (match) match.cardIds.forEach(id => { priceMap[id] = mkt; });
        }
      });
  ...
  // Cache and apply
  try { localStorage.setItem(PRICE_CACHE_KEY, JSON.stringify({ ts: Date.now(), prices: priceMap })); } catch {}
  applyPriceCache(priceMap);
}

function applyPriceCache(priceMap) {
  let updated = 0;
  S.store.cards.forEach(c => {
    if (priceMap[c.id] !== undefined) { c.market = priceMap[c.id]; updated++; }
  });
  S.collection.cards.forEach(c => {
    if (priceMap[c.id] !== undefined) c.market = priceMap[c.id];
  });
  if (updated > 0) {
    renderAll();
    if (S.user) renderCollection();
    // Update live price badge
    const badge = document.getElementById('livePriceBadge');
    if (badge) {
      badge.textContent = '✓ Live prices loaded';
...
window.onload = async () => {
  ...
  // Fetch live prices in background — updates tiles automatically when done
  refreshLivePrices();
};
...
/* ══ TICKER ══ */
// Tickers reference card IDs so clicking them opens the detail modal
const TICKERS = [
  {n:'Charizard ex SIR',     p:380,  d:'up',pct:'+3.2%',g:'poke',i:'🏆',id:'p12'},
  {n:'Umbreon VMAX Alt Art', p:1650, d:'dn',pct:'-1.2%',g:'poke',i:'🏆',id:'p14'},
  {n:'Rayquaza VMAX Alt Art',p:420,  d:'up',pct:'+2.3%',g:'poke',i:'🏆',id:'p15'},
  {n:'Charizard ex 151',     p:28,   d:'up',pct:'+1.8%',g:'poke',i:'⚡',id:'p6b'},
  {n:'Mew ex 151',           p:45,   d:'up',pct:'+4.1%',g:'poke',i:'⚡',id:'p6d'},
  {n:'Iono SAR PSA 10',      p:280,  d:'dn',pct:'-2.5%',g:'poke',i:'🏆',id:'p15a'},
  {n:'Charizard Holo 1st Ed',p:10200,d:'up',pct:'+1.1%',g:'poke',i:'🏆',id:'p10'},
  {n:'Shanks OP01 SEC',      p:35,   d:'up',pct:'+5.2%',g:'op',  i:'⚓',id:'o3a'},
  {n:'Ace OP02 PSA 10',      p:310,  d:'up',pct:'+6.1%',g:'op',  i:'🏆',id:'o11a'},
  {n:'Yamato OP04 CGC 10',   p:180,  d:'up',pct:'+3.8%',g:'op',  i:'🏆',id:'o11b'},
  {n:'Law OP05 PSA 10',      p:220,  d:'dn',pct:'-1.4%',g:'op',  i:'🏆',id:'o11c'},
  {n:'OP01 Booster Box',     p:175,  d:'dn',pct:'-2.1%',g:'op',  i:'📦',id:'o6'},
  {n:'151 Elite Trainer Box',p:60,   d:'up',pct:'+1.5%',g:'poke',i:'📦',id:'p8'},
  {n:'Charizard ex JPN PSA10',p:580, d:'up',pct:'+4.2%',g:'poke',i:'🏆',id:'jp7'},
  {n:'Mew ex JPN SAR',       p:55,   d:'dn',pct:'-1.8%',g:'poke',i:'⚡',id:'jp6'},
];
function buildTicker() {
  const d = [...TICKERS,...TICKERS,...TICKERS];
  document.getElementById('tickerInner').innerHTML = d.map(t =>
    `<span class="t-item" onclick="openCard('${t.id}')" style="cursor:pointer;">
      <span class="t-thumb ${t.g}">${t.i}</span><span class="tn">${t.n}</span>${fmt(t.p,0)}<span class="${t.d}">${t.pct}</span><span class="t-sep"> | </span>
    </span>`
  ).join('');
}
...
function renderHomeGrid() {
  ...
  const modeEl = document.getElementById('apiModeIndicator');
  if (modeEl) {
    if (CONFIG.justTcg.connected) {
      modeEl.innerHTML = '<span style="color:#16A34A">● Live</span> JustTCG connected';
      modeEl.style.background = 'rgba(22,163,74,0.1)';
    } else {
      modeEl.innerHTML = '<span style="color:#D97706">● Demo</span> Sample data — <a onclick="openAdmin()" style="cursor:pointer;color:var(--blue);text-decoration:underline">connect JustTCG API</a> for live pricing';
      modeEl.style.background = 'rgba(217,119,6,0.08)';
    }
  }
  ...
  return `<div class="coll-tile tile" data-card-id="${card.id}" onclick="openCollCard('${card.id}')">
    ...
    <div class="tile-info">
      ...
      <div class="coll-tile-actions">
        ${card.cat==='raw' ? `<button class="cta-btn grade-btn" title="Upgrade to graded slab" onclick="event.stopPropagation();openGradeModal('${card.id}')">🏆 Grade</button>` : ''}
        ${card.cat==='slab' ? `<button class="cta-btn grade-btn graded" title="Edit grading" onclick="event.stopPropagation();openGradeModal('${card.id}')">${card.grader} ${card.grade}</button>` : ''}
...
async function loadPriceChart(card, days) {
  ...
  if (card.cat === 'slab') {
    ...
    dataSource = '📊 Simulated eBay Data (demo mode)';
  ...
  if (srcEl) {
    if (card.cat === 'slab') {
      const saleCount = ebayBubbles.length;
      srcEl.textContent = `📊 Simulated eBay data · ${saleCount} demo sales · Connect eBay API for live data`;
    } else {
      srcEl.textContent = CONFIG.justTcg.connected ? '📊 JustTCG Live Market Data' : '📊 Demo pricing · Connect JustTCG for live data';
    }
  }
...
async function searchJustTCG() {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.pokemontcg.io/v2/cards?q=${encodeURIComponent(qStr)}&select=id,name,set,number,images,tcgplayer,cardmarket&orderBy=-set.releaseDate&pageSize=20`,
      { signal: AbortSignal.timeout(6000) }
    );
    const data = await res.json();
    if (data.data?.length) {
      results = data.data.map(c => {
        // ROOT CAUSE FIX: TCGPlayer price key varies by card type:
        //   ex/V/VMAX => "holofoil", Full Art trainers (Iono 185) => "special",
        //   SIR/SAR => "ultraPremium" or "special", vintage => "1stEditionHolofoil",
        //   commons => "normal" or "reverseHolofoil"
        // Fix: scan ALL price keys, collect every non-zero market value, take the MAX.
        const prices = c.tcgplayer?.prices || {};
        const allMarkets = Object.values(prices).map(p => p?.market || 0).filter(v => v > 0);
        const tcgMkt = allMarkets.length > 0 ? Math.max(...allMarkets) : 0;
        // Fallback: CardMarket averageSellPrice (EUR) converted ~USD
        const cmMkt = c.cardmarket?.prices?.averageSellPrice
          ? Math.round(c.cardmarket.prices.averageSellPrice * 1.08 * 100) / 100
          : 0;
        const mkt = tcgMkt || cmMkt || 0;
        return {
          id: c.id, name: c.name,
          set: c.set?.name || '',
          game: 'pokemon',
...
function confirmManualAdd() {
  ...
  // Always added as raw — user can upgrade to slab via the "Grade this card" button on the tile
  const cat = 'raw';
  ...
  S.collection.cards.push({
    id:'cc'+Date.now(), name:S.manualCard.name, set:setName,
    num:S.manualCard.num||'', game, cat, grader:null, grade:null,
    market:S.manualCard.market||0, paid, qty, portfolio:portId,
    imageUrl:S.manualCard.imageUrl||null
  });
  S.manualCard=null;
  document.getElementById('manualFormArea').style.display='none';
  document.getElementById('manualSearch').value='';
  ...
  closeModal('addCardModal'); renderCollection(); showToast('Card added as raw — tap 🏆 Grade to upgrade','📚');
}

/* ══ UPGRADE TO SLAB MODAL ══ */
function openGradeModal(cardId) {
  const card = S.collection.cards.find(c => c.id === cardId);
  if (!card) return;
  document.getElementById('gradeCardName').textContent = card.name + ' · ' + card.set;
  document.getElementById('gradeCardId').value = cardId;
  // Pre-fill if already graded
  const graderSel = document.getElementById('gradeGrader');
  const gradeInp  = document.getElementById('gradeGrade');
  const paidInp   = document.getElementById('gradePaid');
  if (graderSel) graderSel.value = card.grader || 'PSA';
  if (gradeInp)  gradeInp.value  = card.grade  || '';
  if (paidInp)   paidInp.value   = card.paid   || '';
  openModal('gradeCardModal');
}
function confirmGrade() {
  const cardId = document.getElementById('gradeCardId').value;
  const card   = S.collection.cards.find(c => c.id === cardId);
  if (!card) return;
  const grader = document.getElementById('gradeGrader').value;
  const grade  = parseFloat(document.getElementById('gradeGrade').value);
  const paid   = parseFloat(document.getElementById('gradePaid').value);
  if (!grader || !grade) { showToast('Select grader and grade','⚠️','err'); return; }
  card.cat    = 'slab';
  card.grader = grader;
  card.grade  = grade;
  if (!isNaN(paid) && paid > 0) card.paid = paid;
  // Clear cached image so it re-fetches with correct slab data
  delete IMG_CACHE[card.id];
  closeModal('gradeCardModal');
  renderCollection();
  showToast(`${card.name} upgraded to ${grader} ${grade} slab 🏆`, '✓');
}
function removeGrade(cardId) {
  const card = S.collection.cards.find(c => c.id === cardId);
  if (!card) return;
  card.cat = 'raw'; card.grader = null; card.grade = null;
  delete IMG_CACHE[card.id];
  closeModal('gradeCardModal');
  renderCollection();
  showToast('Card reverted to raw', '✓');
}
